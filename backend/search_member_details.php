...
if($_REQUEST['id']) {
  ...
  try{
    $stmt=$pdo->prepare("SELECT name, phone, phone2, address, occupation, dob, age, enquiry_id, reference_from_media FROM enquiry WHERE id='".$_REQUEST['id']."'");
     }
     ...
     $user = $stmt->fetch();
      ...
      $array_toto["enquiry_id"] = $user['enquiry_id'];

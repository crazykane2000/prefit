<?php session_start();
     ...
     else 
    {
      if (move_uploaded_file($_FILES["upload_img"]["tmp_name"], $target_file)) {
        ...
        $table = "staff";
         try {
                $stmt = $pdo->prepare('UPDATE `'.$table.'` SET `type`="'.$_REQUEST['type'].'", `name`="'.$_REQUEST['name'].'", `email`="'.$_REQUEST['email'].'", `password`="'.$_REQUEST['password'].'", `mobile`="'.$_REQUEST['mobile'].'", `dob`="'.$_REQUEST['dob'].'", `age`="'.$_REQUEST['age'].'", `salary`="'.$_REQUEST['salary'].'", `address`="'.$_REQUEST['address'].'", `upload_img`="'.$image_file.'" , `joining_date`="'.$_REQUEST['joining_date'].'", `status`="'.$_REQUEST['status'].'"WHERE `id` = '.$id);
